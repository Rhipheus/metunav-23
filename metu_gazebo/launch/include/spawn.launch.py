...
def generate_launch_description():
    ...
    diff_controller_spawner = Node(
        ...
        executable="spawner", #spawner.py for foxy
        #remappings=[("/odom","/odom_ros2_control")],
    ...
    joint_state_broadcaster_spawner = Node(
        package="controller_manager",
        executable="spawner", #spawner.py for foxy
        #remappings=[("/odom","/odom_diff_joint")],
    ...
    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory(
                "metu_description"), "launch", "robot_state_publisher.launch.py")
        )
    )
    ...
    ld.add_action(diff_controller_spawner)
